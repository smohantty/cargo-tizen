...
pub fn run_gh_release(ctx: &AppContext, args: &GhReleaseArgs) -> Result<()> {
    ...
    // Step 4: Stage binaries to tizen/<arch>/
    if !plan.no_stage {
        for &arch in &plan.arches {
            stage_binaries(ctx, &plan, arch)?;
        }
    }
    ...
    // Step 7: Commit staged artifacts (and spec if synced)
    {
        let mut paths_to_add: Vec<String> = Vec::new();
        if !plan.no_stage {
            for &arch in &plan.arches {
                let arch_dir = plan.packaging_root.join(arch.as_str());
                let arch_rel = arch_dir
                    .strip_prefix(&plan.workspace_root)
                    .unwrap_or(&arch_dir);
                paths_to_add.push(format!("{}/", arch_rel.display()));
            }
        }
        if spec_synced {
...
fn resolve_config(ctx: &AppContext, args: &GhReleaseArgs) -> ResolvedConfig {
    ...
    let package = args
        ...
        .or_else(|| ctx.config.default_package().map(String::from));
...
fn stage_binaries(ctx: &AppContext, plan: &ReleasePlan, arch: Arch) -> Result<()> {
    let use_color = color_enabled();
    ...
    if !binary_path.exists() {
        bail!(
            "built binary not found: {}\n\
             expected after `cargo tizen build -A {} --release`",
            binary_path.display(),
            arch.as_str()
        );
    }

    let dest_dir = plan.packaging_root.join(arch.as_str());
    fs::create_dir_all(&dest_dir)
        .with_context(|| format!("failed to create {}", dest_dir.display()))?;

    let dest = dest_dir.join(&plan.package_name);
    fs::copy(&binary_path, &dest).with_context(|| {
        format!(
